...
with Dataset("gpp.nc", mode="w") as dset:
    ...
    # data
    D = dset.createVariable("gpp", data.dtype, ("time", "data"), fill_value = -999)
    D[...]          = data
    D.units         = "kg/m2/s"
    ...
    dset.title = "Gross Primary Production, from Daytime partitioning method, reference selected from GPP versions using model efficiency (MEF). The MEF analysis is repeated for each time aggregation"
