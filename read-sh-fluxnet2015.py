...
gist_source = "https://github.com/mmu2019/Datasets/blob/master/read-sh-fluxnet2015.py"
...
#data   = np.ma.masked_array(np.random.rand(t.size, nfile))
#data2D = np.ma.masked_array(np.random.rand(nyears,nmonth))
...
with Dataset("sh.nc", mode="w") as dset:
    ...
    # data
    D = dset.createVariable("sh", data.dtype, ("time", "data"), fill_value = -999)
    D[...]          = data
    D.units         = "W/m2"
    ...
    dset.title = "Sensible heat flux, gapfilled using MDS method"
    dset.version = "2015"
